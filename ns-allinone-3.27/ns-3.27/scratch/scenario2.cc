...
int main (int argc, char *argv[])
{
  ...
  bool verbose = false;
  double simulationTime = 36.0;                        /* Simulation time in seconds. */
...
/** Generating traffic parameters **/
  ...
  uint32_t dataSize = 5000;   // bytes, for applications
  ...
  rpi01ReliabilityHelper.SetApplication("LinearRegression",dataSize,packetSize);
  ...
  rpi0otherReliabilityHelper.SetApplication("LinearRegression",infDataSize,packetSize);
  ...
  for (uint8_t index = 1; index < rpi0Nodes.GetN (); ++index)
    {
          auto ipv4 = rpi0Nodes.Get (0)->GetObject<Ipv4> ();
          const auto address = ipv4->GetAddress (1, 0).GetLocal ();
          InetSocketAddress sinkSocket (address, portNumber++);
          OnOffHelper onOffHelper ("ns3::UdpSocketFactory", sinkSocket);
          onOffHelper.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=.12]"));
          ...
          sourceApplications.Add (onOffHelper.Install (rpi0Nodes.Get (index)));
          PacketSinkHelper packetSinkHelper ("ns3::UdpSocketFactory", sinkSocket);
          sinkApplications.Add (packetSinkHelper.Install (rpi0Nodes.Get (0)));
    }
...
//  Simulator::Schedule (Seconds (1.1), &CalculateThroughput);
  ...
  onOffHelper.SetAttribute ("DataRate", DataRateValue (DataRate (dataRate3)));
  onOffHelper.SetAttribute ("PacketSize", UintegerValue (packetSize)); //bytes
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      if ((t.sourceAddress == Ipv4Address("172.27.0.3") && t.destinationAddress == Ipv4Address("172.27.0.1"))
         || (t.sourceAddress == Ipv4Address("172.27.0.4") && t.destinationAddress == Ipv4Address("172.27.0.3"))
         || (t.sourceAddress == Ipv4Address("172.27.0.5") && t.destinationAddress == Ipv4Address("172.27.0.3"))
         || (t.sourceAddress == Ipv4Address("172.27.0.6") && t.destinationAddress == Ipv4Address("172.27.0.3")))
        {
