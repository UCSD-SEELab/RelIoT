...
PrintInfo (NodeContainer nodes)
{
  ...
  for(uint32_t i=0;i<nodes.GetN();i++)
  {
    ...
    power_vec = nodes.Get(i)->GetObject<PowerTraceModel>()->GetPower();
...
main (int argc, char **argv)
{
  ...
  rapidcsv::Document doc("ptrace_normal.csv");
  std::vector<float> cpu_power = doc.GetColumn<float>("cpu_pwr");
  std::vector<float> gpu_power = doc.GetColumn<float>("gpu_pwr");
  ...
  reliabilityHelper.SetPowerModel("PowerTraceModel");
  reliabilityHelper.SetTrace(cpu_power,gpu_power);
  reliabilityHelper.SetSamplingPeriod(DoubleValue(0.2));
 ...
 Simulator::Schedule (now,&PowerTraceModel::UpdatePower,nodes.Get (i)->GetObject<PowerTraceModel> ());
  now += Seconds (800);
