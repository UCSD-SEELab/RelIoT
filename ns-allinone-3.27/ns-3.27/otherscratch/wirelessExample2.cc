...
int main (int argc, char *argv[])
{
 ...
 // int nBottomNodes = rows * columns;
  ...
  // Create nodes
  NodeContainer fixedPosTop;
  //NodeContainer fixedPosBottom;

  fixedPosTop.Create(nTopNodes); //fixedpos.Create(bottomrow);
  ...
  NetDeviceContainer fixedPosTopDev = wifi.Install (wifiPhyHelper, wifiMacHelper, fixedPosTop);
 ...
 // Ptr<ListPositionAllocator> positionAlloc2 = CreateObject<ListPositionAllocator> (); // Top nodes
  ...
  for(int j=0; j< columns; j++){
   ...
   mobilityFixedNodes.Install (fixedPosTop);
 ...
 /////////////-------- Energy Source and Device Energy Model  configuration -----------------------------------
  ...
  // install source
  EnergySourceContainer sources = basicSourceHelper.Install (fixedPosTop);
  /* device energy model */
...
// packets are marked in the tracefile with ns3::SeqTsHeader, including sequence numbers
 ...
 // lowerleft->AddApplication(UdpSendApp);
  ...
  for(int i=0; i< numClients; i++){
	  ...
	  Ptr<Node> clientNode = fixedPosTop.Get(rand() % totalNodes);
